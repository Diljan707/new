...
# Limits removed: Ab saare channels aur optimal max workers use honge
...
def generate_safe_playlist_all():
  print(
      "[*] Downloading target playlist and extracting keys for ALL channels..."
  )
  try:
    ...
    for i, line in enumerate(lines):
      if line.strip().startswith("#EXTINF"):
        ...
        # Limit break kar di gayi hai, ab saare add honge
    ...
    print(
        f"[*] Found {len(target_indices)} channels. Processing all concurrently"
        "..."
    )

    channel_results = {}
    # Worker limit remove krne ke liye dynamic max_workers (jaise 50 ya thread pool default) set kiya hai
    with ThreadPoolExecutor(
        max_workers=min(100, len(target_indices))
    ) as executor:
      futures = {
    ...
    output_file = "all_safe_channels.m3u"
    with open(output_file, "w", encoding="utf-8") as f:
      f.write("\n".join(new_lines))

    print(
        f"\n[+] Success! Total {len(channel_results)} channels saved as"
        f" '{output_file}'."
    )
